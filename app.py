...
def answer_question(context, question):
    ...
    answer = tokenizer_qa.decode(inputs['input_ids'][0, start_index:end_index])
...
# Streamlit app
def main():
    st.title("A Showcase of Progressive QA and Sentence Embeddings")
    ...
    st.write(f"Using the question: '{question}' for sentence embeddings visualization.")
    
    if st.button("Visualize Embeddings"):
        embeddings = encode_sentence(question)  # Use the entered question for embeddings
        tokens = tokenizer_emb.tokenize(tokenizer_emb.decode(tokenizer_emb.encode(question)))
